# Gmail Fetcher Implementation Plan v5

**Document ID**: 0109-1900_implementation_plan_v5.md
**Release Version**: 2.0.0
**Status**: Executable (all bugs from v4 review fixed)
...
|----------|---------|-------|
| This document | v5 (internal revision) | |
...
.SYNOPSIS
    ...
    v5: no changes needed (already counts occurrences)
#>
[CmdletBinding()]
param(
...
function Get-SysPathInsertCount {
    # Counts OCCURRENCES (not files) to match baseline.sh
    $count = 0
...
function Get-ConfigLocationCount {
    $locations = @("config/app", "config/analysis", "src/analysis")
...
# Build JSON object
$baseline = [ordered]@{
    schema_version = "1.1"  # v5: added depth_convention
    depth_convention = "segments_under_repo_root"  # v5: explicit semantics
    timestamp = $timestamp
...
### `scripts/audit/baseline.sh` (v5 Fixed)
...
# Captures baseline measurements and writes JSON atomically.
# v4: uses find -prune
# v5: Fixed sys.path metric to count OCCURRENCES not FILES
#
set -euo pipefail
...
COMMIT_SHA="$(git rev-parse HEAD 2>/dev/null || echo "no-commits")"
...
# v5 FIX: Count OCCURRENCES not FILES (match PowerShell behavior)
get_sys_path_insert_count() {
    ...
    \) -prune -o -name "*.py" -print0 2>/dev/null | \
        xargs -0 grep -ohE 'sys\.path\.(insert|append)' 2>/dev/null | \
        wc -l | \
        tr -d '[:space:]'
}
...
# v5: Added schema_version 1.1 and depth_convention
cat > "$TEMP_FILE" << EOF
{
  "schema_version": "1.1",
...
## 2. Configuration Loader (v5 Fixes)
...
- Removed dead try/except around is_relative_to (doesn't raise ValueError)
"""
...
class AppConfig:
    ...
    def _load_from_file(
        ...
        # v5 FIX: Check BOTH config dir repo AND CWD repo for credential safety
    ...
    def _find_all_repo_roots(cls, config_dir: Path) -> list[Path]:
        """
        v5 FIX: Find repo roots from BOTH config directory AND CWD.
        ...
        # Check from CWD (catches case where config is outside repo but we're running in one)
    ...
    def _check_path_safety(
        ...
        # v5 FIX: is_relative_to returns bool, doesn't raise ValueError
        # No try/except needed - it handles cross-drive cases by returning False
...
## 3. Import Policy Checker (v5 Fixes)
...
def check_relative_import(
    ...
    """
    v5 FIX: Validate relative imports don't escape package boundary.
    ...
    if file_depth < 0:
        # File not under package root - relative imports shouldn't be used
        return (
...
def main() -> int:
    ...
    if src_pkg.exists():
        ...
        src_dir = Path("src")
        if src_dir.exists():
            check_dirs.append((src_dir, None))  # No package root for relative checks
...
## 4. pyproject.toml (v5 Fixed Hatchling Config)
...
# v5 FIX: Correct Hatchling src-layout configuration
# Use import name + sources, not filesystem path in packages
...
# Ensure py.typed is included in wheel
...
jobs:
  ...
  # v5 FIX: Build validation with proper dist cleanup and single wheel selection
  build-validation:
    ...
    steps:
      ...
      # v5 FIX: Clean dist before build to ensure single wheel
      ...
      # v5 FIX: Print __file__ to verify correct package location
...
# v5 FIX: Properly handle old src/__init__.py (tracked OR untracked)
...
## 9. Release DoD Checks (v5 Fixed Path Handling)
...
# v5 FIX: Capture repo root at start, use throughout
$repoRoot = (git rev-parse --show-toplevel)
if (-not $repoRoot) {
...
try {
    ...
    # v5 FIX: Always clean dist/ before build
    ...
    # v5 FIX: Select wheel by exact pattern, verify single wheel
    ...
    # v5 FIX: Import checks from temp dir using absolute venv paths
...
try {
    ...
    # v5 FIX: Run tests from temp directory using absolute paths
    $tempDir = [System.IO.Path]::GetTempPath()
    $testsPath = Join-Path $repoRoot "tests"
...
## 10. Summary: All v4 Issues Fixed

| # | Issue | Fix |
|---|-------|-----|
| 1 | Hatchling config wrong | `packages = ["gmail_fetcher"]` + `sources = ["src"]` |
| 2 | baseline.sh counts files not occurrences | `grep -ohE` to count matches |
| 3 | Config repo-safety misses CWD | `_find_all_repo_roots()` checks both config dir and CWD |
| 4 | is_relative_to dead try/except | Removed - method returns bool, doesn't raise |
| 5 | No relative import validation | `check_relative_import()` validates level vs depth |
| 6 | Bad error messages for 'src' | Special-case INVALID_IMPORT_ROOTS with clear message |
| 7 | CI multiple wheels possible | Clean dist/ before build, verify single wheel |
| 8 | release_checks.ps1 broken paths | Absolute paths via `$repoRoot`, `Push/Pop-Location` |
| 9 | git rm fails silently for untracked | Check `git ls-files`, use `Remove-Item` for untracked |
| 10 | Depth semantics undocumented | `schema_version: 1.1`, `depth_convention` field |

---

**Document Version**: v5
**Release Version**: 2.0.0
**Status**: Executable - all v4 bugs fixed
