# Gmail Fetcher Implementation Plan v8

**Document ID**: 0109-2300_implementation_plan_v8.md
**Release Version**: 2.0.0
**Status**: Executable (all bugs from v7 review fixed)
...
|----------|---------|-------|
| This document | v8 (internal revision) | |
...
| # | Issue | Fix |
|---|-------|-----|
| 1 | Wheel glob assumes `gmail_fetcher-*.whl` filename | Select by counting `*.whl` files, require exactly 1 |
| 2 | baseline.ps1 entry_points=2 but measures 4 possible paths | Split post-migration vs legacy metrics |
| 3 | Get-ConfigLocationCount counts arbitrary folders | Split post-migration vs legacy config locations |
| 4 | CI baseline step calls `pwsh` inside `shell: pwsh` | Simplified to direct invocation |
| 5 | Config loader allows empty string paths | Added `strip()` validation |
| 6 | Config loader doesn't expand `~` | Added `expanduser()` call |
...
## 1. Baseline Script (v8 Fixes - Split Metrics)
...
.SYNOPSIS
    ...
    v7: Updated paths for post-migration layout
    v8: Split post-migration vs legacy metrics to avoid false failures
#>
[CmdletBinding()]
param(
...
# v8 FIX: Split post-migration vs legacy config locations
function Get-PostMigrationConfigLocationCount {
    $locations = @(
        "src/gmail_fetcher/analysis"     # post-migration config/analysis location
    )
    return ($locations | Where-Object { Test-Path (Join-Path $repoRoot $_) }).Count
}

function Get-LegacyConfigLocationCount {
    $legacy = @(
        "config/app",
        "config/analysis",
        "src/analysis"                   # legacy pre-migration location
    )
...
# v8 FIX: Split post-migration vs legacy entry points
...
function Get-LegacyEntryPointCount {
    $legacy = @(
        ...
        "src/cli/main.py"       # legacy pre-migration location
    )
    return ($legacy | Where-Object { Test-Path (Join-Path $repoRoot $_) }).Count
}
...
# v8: Separate post-migration and legacy metrics
$measurements = [ordered]@{
    max_folder_depth = Get-MaxFolderDepth
    sys_path_inserts = Get-SysPathInsertCount
    post_migration_config_locations = Get-PostMigrationConfigLocationCount
    legacy_config_locations = Get-LegacyConfigLocationCount
...
# Build JSON object
# v8: Updated targets with split metrics
#   - post_migration_entry_points = 2 (cli/main.py + __main__.py)
#   - legacy_entry_points = 0 (should be removed post-migration)
#   - post_migration_config_locations = 1 (only src/gmail_fetcher/analysis)
#   - legacy_config_locations = 0 (should be removed post-migration)
$baseline = [ordered]@{
    schema_version = "1.3"  # v8: bumped for split metrics
    ...
    targets = [ordered]@{
        max_folder_depth = 3
        sys_path_inserts = 0
        post_migration_config_locations = 1   # v8: split metric
        legacy_config_locations = 0           # v8: should be 0 post-migration
        post_migration_entry_points = 2       # v8: split metric (cli/main.py + __main__.py)
        legacy_entry_points = 0               # v8: should be 0 post-migration
        hidden_docs = 0
    }
}
...
## 2. Configuration Loader (v8 Fixes - Empty String + Expanduser)
...
v8 fixes:
- Added empty string validation (rejects whitespace-only paths)
- Added expanduser() for ~ expansion on all platforms
"""
...
class AppConfig:
    ...
    def _load_from_file(
        ...
        def resolve_path(key: str, default: Path) -> Path:
            ...
            # v8 FIX: Reject empty or whitespace-only paths
            if not raw.strip():
                raise ConfigError(f"{key} must be non-empty string path")
            # v8 FIX: Expand ~ to user home directory
...
(No changes needed for v6/v7/v8)
"""
from __future__ import annotations
...
(No changes needed for v5/v6/v7/v8)
"""
from __future__ import annotations
...
## 7. CI Workflow (v8 - Fixed Wheel Selection + Simplified Baseline)
...
jobs:
  ...
  # v8 FIX: Select wheel by counting, not by name pattern
  build-validation:
    ...
    steps:
      ...
      - name: Verify single wheel exists and install
        run: |
          ...
          /tmp/test-venv/bin/pip install "$WHEEL_PATH"
  ...
  # v8 FIX: Simplified - shell: pwsh already handles the invocation
  baseline-check:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      ...
      - name: Run baseline measurements
        shell: pwsh
        run: .\scripts\audit\baseline.ps1 -OutputDir "docs/audit"
...
## 9. Release DoD Checks (v8 - Fixed Wheel Selection)
...
v8 fixes:
- Select wheel by counting *.whl files, not by name pattern
  (Wheel filename normalization varies: gmail_fetcher vs gmail-fetcher)
#>
[CmdletBinding()]
param()
...
try {
    ...
    # v8 FIX: Select wheel by counting *.whl files, not by name pattern
    $wheels = Get-ChildItem -Path $distPath -Filter "*.whl" -ErrorAction Stop
...
## 10. Summary: All v7 Issues Fixed in v8

| # | Issue | Fix |
|---|-------|-----|
| 1 | Wheel glob assumes `gmail_fetcher-*.whl` | Count `*.whl` files, require exactly 1 |
| 2 | entry_points=2 contradicts 4-path measurement | Split `post_migration_entry_points` (target: 2) vs `legacy_entry_points` (target: 0) |
| 3 | Get-ConfigLocationCount counts arbitrary dirs | Split `post_migration_config_locations` (target: 1) vs `legacy_config_locations` (target: 0) |
| 4 | CI baseline calls `pwsh` inside `shell: pwsh` | Simplified to direct `.\scripts\audit\baseline.ps1` invocation |
| 5 | Config allows empty string paths | Added `if not raw.strip():` validation |
| 6 | Config doesn't expand `~` | Added `Path(raw).expanduser()` |

### Baseline Targets (v8 - Split Metrics)
...
| `post_migration_config_locations` | 1 | Only `src/gmail_fetcher/analysis` |
| `legacy_config_locations` | 0 | Legacy dirs should be removed |
| `post_migration_entry_points` | 2 | `cli/main.py` + `__main__.py` |
| `legacy_entry_points` | 0 | Legacy entry points should be removed |
| `hidden_docs` | 0 | No docs/claude-docs files |

---

## v8 Verification Commands

### Wheel selection sanity (cross-platform)
```powershell
python -m build
$wheels = Get-ChildItem dist -Filter *.whl
$wheels | ForEach-Object { Write-Host $_.Name }
if ($wheels.Count -ne 1) { throw "Expected 1 wheel, found $($wheels.Count)" }
```

### Baseline with split metrics
```powershell
.\scripts\audit\baseline.ps1 -OutputDir docs/audit
$latest = Get-ChildItem docs/audit/*_baseline.json | Sort-Object LastWriteTime | Select-Object -Last 1
Get-Content $latest.FullName | ConvertFrom-Json | ConvertTo-Json -Depth 5
```

### Config type-hardening tests
```powershell
...
# Expect: ConfigError credentials_path must be non-empty string path
...
'@ | Set-Content -Encoding utf8 .\tilde_config.json

python -c "from pathlib import Path; from gmail_fetcher.core.config import AppConfig;
import os
try:
    cfg = AppConfig.load(Path('tilde_config.json'), allow_repo_credentials=True)
    print('Expanded to:', cfg.credentials_path)
    assert str(cfg.credentials_path).startswith(os.path.expanduser('~'))
except Exception as e: print(type(e).__name__, e)"
# Expect: Expanded to: C:\Users\<user>\creds.json (or /home/<user>/creds.json)

Remove-Item .\tilde_config.json
```
...
**Document Version**: v8
**Release Version**: 2.0.0
**Status**: Executable - all v7 bugs fixed
