# Gmail Fetcher Architecture Restructuring - Final Implementation Plan (Complete)

**Document ID**: 0109-2330_final_implementation_plan_complete.md
**Date**: 2026-01-09
**Version**: Final (consolidated from v1-v7 with ALL executable code)
...
3. [Critical Decisions (Locked)](#3-critical-decisions-locked)
4. [Platform Contract](#4-platform-contract)
5. [Current State Analysis](#5-current-state-analysis)
6. [Proposed Structure](#6-proposed-structure)
7. [Technical Specifications](#7-technical-specifications)
8. [Migration Phases](#8-migration-phases)
9. [Executable Scripts](#9-executable-scripts)
...
### 1.2 Key Problems Identified (Cross-Agent Consensus)
...
These decisions were locked in v3 and remain unchanged:
...
### 5.1 Folder Hierarchy (Pre-Migration)
...
### 6.2 Navigation Paths Improvement
...
### 7.2 Configuration Loader (Complete - v7)
...
### 7.3 CLI Implementation (Complete - v7)
...
| Task | Notes |
|------|-------|
| Create `pyproject.toml` | Use spec from Section 7.1 |
...
### 9.1 Baseline Measurement Script (v7)
...
### 10.1 Import Policy Checker (Complete - v5)
...
### 10.2 Release DoD Checks (Complete - v5)
...
### 11.1 GitHub Actions Workflow (Complete - v7)
...
### 12.4 Pre-Migration Security Checklist
...
| Version | Key Changes |
|---------|-------------|
| v1 | Initial assessment, current state analysis, proposed structure, reasoning matrix |
| v2 | Added measurement methodology, pyproject.toml spec, security checklist, test organization policy, config loader contract, CLI compatibility |
| v3 | Critical decisions locked (Click CLI, clean break, Hatchling, Python 3.10+), executable PowerShell scripts |
| v4 | Fixed PowerShell exclusion bugs (segment-based matching), config loader security improvements, Hatchling config fixes, schema_version |
| v5 | Fixed bash script metrics, relative import validation, CI build cleanup, absolute path handling in release checks |
| v6 | Fixed `is_relative_to` ValueError (Windows cross-drive paths), removed bash scripts (PowerShell only) |
| v7 | Updated paths for post-migration layout, fixed CI baseline invocation (`pwsh -NoProfile -File`), config type validation before `Path()` |

---

## Verification Commands

### Baseline smoke test (cross-platform)
```bash
pwsh -NoProfile -File scripts/audit/baseline.ps1 -OutputDir docs/audit
...
### Import resolution from temp directory
```powershell
$tempDir = [System.IO.Path]::GetTempPath()
Push-Location $tempDir
python -c "import gmail_fetcher; print(gmail_fetcher.__version__)"
Pop-Location
```
...
**Total Lines**: ~2100 (comprehensive with all executable code)
