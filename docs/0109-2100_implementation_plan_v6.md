# Gmail Fetcher Implementation Plan v6

**Document ID**: 0109-2100_implementation_plan_v6.md
**Release Version**: 2.0.0
**Status**: Executable (all bugs from v5 review fixed)
...
|----------|---------|-------|
| This document | v6 (internal revision) | |
...
## v5 Issues Fixed in v6

| # | Issue | Fix |
|---|-------|-----|
| 1 | `is_relative_to()` raises ValueError on Windows cross-drive | Re-added try/except ValueError |
| 2 | Hatchling config wrong (packages/sources syntax) | `packages = ["src/gmail_fetcher"]`, removed sources |
| 3 | baseline.sh fails under set -e with zero matches | **Removed baseline.sh entirely** - PowerShell-only tooling |

---

## 1. Baseline Script (PowerShell Only)

**v6 DECISION**: No `.sh` scripts. This is a Windows-focused project. PowerShell Core (`pwsh`) is cross-platform if CI ever needs Linux support.
...
## 2. Configuration Loader (v6 Fixes)
...
class AppConfig:
    ...
    def _check_path_safety(
        ...
        # e.g., Path("D:\\x").is_relative_to(Path("C:\\y")) raises ValueError
        try:
...
## 4. pyproject.toml (v6 Fixed Hatchling Config)
...
# packages expects FILESYSTEM PATH, not import name
# The final path component becomes the import name
...
## 7. CI Workflow (v6 - PowerShell Only)
...
jobs:
  test:
    ...
    steps:
      ...
      # v6: Use pwsh for all platforms (cross-platform PowerShell)
  ...
  # v6: Baseline check uses pwsh (cross-platform)
  baseline-check:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
...
## 10. Summary: All v5 Issues Fixed in v6

| # | Issue | Fix |
|---|-------|-----|
| 1 | `is_relative_to()` raises ValueError on Windows cross-drive | Re-added `try/except ValueError` block |
| 2 | Hatchling config wrong (`packages`/`sources` syntax) | `packages = ["src/gmail_fetcher"]`, removed `sources` |
| 3 | baseline.sh fails under `set -e` + unnecessary for Windows project | **Removed baseline.sh entirely** - PowerShell-only tooling |

---

## v6 Verification Commands

### Hatchling wheel structure
```bash
rm -rf dist/ && python -m build
python -c "import zipfile, glob; w=glob.glob('dist/*.whl')[0]; z=zipfile.ZipFile(w); print([n for n in z.namelist() if '__init__' in n])"
# Should show: ['gmail_fetcher/__init__.py', ...]
```

### Cross-drive path safety (Windows)
```powershell
python -c "from pathlib import Path; p=Path(r'D:\x'); r=Path(r'C:\y'); print('no crash' if not any([p.is_relative_to(r) if hasattr(Path,'is_relative_to') else False]) else 'ok')"
# Actually test with the config loader
```

### Baseline on Linux (uses pwsh)
```bash
pwsh ./scripts/audit/baseline.ps1
```
...
**Document Version**: v6
**Release Version**: 2.0.0
**Status**: Executable - all v5 bugs fixed
